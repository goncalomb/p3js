export class ROMReaderWriter {
  ...
  static _parseROMValues(text, k_limit, v_limit) {
    ...
    for (let i = 0, l = lines.length; i < l; i++) {
      ...
      if (matches) {
        ...
        if (k < 0 || k > k_limit) {
          throw new Error('Invalid index ' + matches[1] + ' expecting 0 to ' + k_limit.toString(16) + ' (hex), on line ' + (i + 1));
        }
        if (values[k] !== undefined) {
          throw new Error('Duplicate index ' + matches[1] + ', on line ' + (i + 1));
        }
        let v = parseInt(matches[2], 16);
        if (v < 0 || v > v_limit) {
          throw new Error('Invalid value ' + matches[2] + ' expecting 0 to ' + v_limit.toString(16) + ' (hex), on line ' + (i + 1));
        }
        values[k] = v;
      } else {
  ...
  overwriteROMContents(string) {
    let new_values = this.constructor._parseROMValues(string, this.size - 1, 2 ** this.wordLength - 1);
